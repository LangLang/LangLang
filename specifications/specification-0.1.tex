...
Therefore this rule may be implemented using a more complex mechanism:

\[
\infer{\left(\dfrac{ctx \vdash e}{\vdots} \quad \dfrac{\left(\dfrac{ctx \vdash es \rightarrow e}{\vdots}\right).\top}{\vdots} \quad \dfrac{ctx \vdash es}{\vdots} \quad \dfrac{\left(\dfrac{ctx \vdash e \rightarrow es}{\vdots}\right).\top}{\vdots}\right)}
{ctx \vdash (e\ es)} \eqno{1.4.a}
\]

\emph{TODO: This rule will still have some trouble when assertions are introduced into the collection. Need to factor in type-checking later on.
Also note the denotational invariants regarding duplicate symbols/expressions in a collection.}
