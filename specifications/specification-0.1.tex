...
\[
\infer{\dfrac{ctx \vdash exs_{0_1}}{\vdots}}{(ctx \vdash \lambda \rightarrow exs_{0_1}).(\vdash \top)} \eqno{3.?.1}
\]

\[
\infer{\left(\dfrac{(ctx_0 \vdash \lambda \rightarrow e_{0_1}).(\vdash exs_1)}{\vdots} \quad \dfrac{(ctx_0 \vdash \lambda \rightarrow es_{0_1}).(\vdash exs_1)}{\vdots}\right)}{(ctx_0 \vdash \lambda \rightarrow (e_{0_1}\ es_{0_1})).(ctx_1 \vdash exs_1)} \eqno{3.?.2}
\]
...
\[
\infer{\left(\dfrac{(ctx_0 \vdash \lambda \rightarrow ex_0).(\vdash e_{1_0})}{\vdots} \quad \dfrac{(ctx_0 \vdash \lambda \rightarrow ex_0).(\vdash es_{1_1})}{\vdots}\right)}{(ctx_0 \vdash \lambda \rightarrow ex_0).(ctx_{1_0} \vdash e_{1_0} \quad ctx_{1_1} \vdash es_{1_1})} \eqno{3.?.3}
\]
Keeping the sequent syntax but discarding the internal context, same as rule 3.?.2.


\[
\infer{ctx \vdash ex_0}{(ctx \vdash \lambda \rightarrow ex_0).ex_0} \eqno{3.?.4}
\]
...
\emph{TODO: Also, when this rule is fixed it's likely the internal context of the right-hand side will need to be retained in order to test equality of functions... (through co-evalation?).
