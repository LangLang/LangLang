...
\emph{NOTE: for now rule 2 is grayed out as many of its rules are being integrated into rule 3.
Specifically we want control over the way the sequent behaves inside rules (e.g. see the rules 2.2, 2.6 etc) to be given to query rules.
This seems to make sense as we expect all progress in the state of the program to be a result of evaluation (i.e. queries).}
...
\[
\infer{\dfrac{\left(\left(\left(\lambda \rightarrow \left(\dfrac{ctx \vdash exs_{0_0}}{\vdots}\right)\right).(\vdash \top)\right) \rightarrow exs_{0_1}\right).\top}{\vdots}}
{(ctx \vdash exs_{0_0} \rightarrow exs_{0_1}).\top} \eqno{3.1.5.1.2}
\]

\emph{TODO: For 3.1.5.1.2 to work we need a rule that takes something like $((ctx \vdash exs_{0_0}) \rightarrow exs_{0_1}).exs_1$
where the left-hand side has already been evaluated to something like $(\lambda \rightarrow (ctx, exs_{0_0} \rightarrow exs_{0_1} \vdash exs_{0_1})).(\dots \vdash exs_1)$.
Well... actually, probably through the intermediate step $(ctx, exs_{0_0} \rightarrow exs_{0_1} \vdash \lambda \rightarrow exs_{0_1}).exs_1$...
Also see the old rules 2.2, 2.5 and 2.6.}
